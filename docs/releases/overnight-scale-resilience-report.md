...
passes here (each was run both ways); **MEASURED LOCALLY** — observed on this laptop,
...
| D6 | **Repeated lifecycles: no retained state found** — see §6. | — | test added | PROVEN BY TEST (clean on base and branch) |
| D7 | **The capacity sampler could not see a run that did not fit.** It recorded load, available memory and peaks, but not container OOM kills, pressure stall or inode headroom. | capacity tooling | Adds PSI (cpu/memory/io), Docker-root free inodes, cumulative OOM kills (`docker events --until`, read-only) and `oom.csv`; summary prints them; still no thresholds. | PROVEN BY TEST `scripts/test-production-host-scripts.sh` (53 cases, 0 failed; 2 new) |
