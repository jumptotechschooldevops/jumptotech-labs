...
| Docker daemon restart / host reboot (maintenance window) | `sudo systemctl restart docker` / `sudo reboot` | platform containers return (`unless-stopped`) unless an operator had stopped them. **Unmeasured:** whether the kind node container returns and becomes Ready, and whether the attestation still validates. The preflight records the node's restart policy (`kind.node-restart-policy`, WARN when it is `no`); record its state after the drill with `docker inspect -f '{{.State.Status}} {{.HostConfig.RestartPolicy.Name}}' jumptotech-labs-control-plane` | NOT PROVEN |
