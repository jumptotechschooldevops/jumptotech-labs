...
export function assertValidSandboxId(value: unknown): string {
  if (!isContainerSandboxRef(value)) throw new InvalidSandboxIdError(String(value));
  return assertValidContainerSandboxRef(value);
}

export function isSandboxId(value: unknown): value is string {
  return isContainerSandboxRef(value);
}
...
export function topologyFor(
  ...
  // The control node *is* the session's sandbox container: an Ansible session
  // is an ordinary sandbox with machines attached, not a parallel world with
  // its own naming. That is what keeps the store, the reaper, the session view
  // and the terminal token all speaking about the same reference.
  const control: AnsibleNode = { name: 'control', role: 'control', container: id };
  const managed = managedNodeNames(managedNodeCount).map<AnsibleNode>((name, index) => ({
    name,
    role: 'managed',
    container: nodeRefForSandbox(id, index + 1),
  }));

  return {
    sandboxId: id,
    network: networkRefForSandbox(id),
    control,
    managed,
...
export function isSandboxObjectName(name: string): boolean {
  return (
    isContainerSandboxRef(name) ||
    /^jtt-node[1-9]-[0-9a-f]{6,40}$/.test(name) ||
    /^jtt-net-[0-9a-f]{6,40}$/.test(name)
  );
}
