...
export class DockerAnsibleSandbox implements AnsibleSandboxPort {
  ...
  readonly #docker: AnsibleExecPort;
  ...
  async ping(sandboxId: string): Promise<void> {
    ...
    if (!info.running) {
      throw new AnsibleSandboxUnreachableError(
        'the control node for this session is not running',
      );
    }
  }
...
function toRunResult(result: AnsibleExecResult): AnsibleRunResult {
  return {
