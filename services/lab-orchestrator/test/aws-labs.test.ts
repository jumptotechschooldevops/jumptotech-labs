...
describe('AWS lab definitions load', () => {
  it('loads AWS-001 with no definition errors anywhere in the catalog', async () => {
    ...
    expect(registry.labsForTrack('aws').map((l) => l.id)).toEqual(['AWS-001', 'AWS-002', 'AWS-003', 'AWS-004', 'AWS-005', 'AWS-006', 'AWS-007', 'AWS-008', 'AWS-009', 'AWS-012', 'AWS-018']);
  });

  it('declares the Linux sandbox, and gets container isolation for free', async () => {
    const registry = await realRegistry();

    for (const id of ['AWS-001', 'AWS-002', 'AWS-003', 'AWS-004', 'AWS-005', 'AWS-006', 'AWS-007', 'AWS-008', 'AWS-009', 'AWS-012', 'AWS-018']) {
...
describe('AWS-001 official documentation validation', () => {
  ...
  it('claims only SOA-C03 for the policy-authoring labs', async () => {
    ...
    for (const id of ['AWS-002', 'AWS-003', 'AWS-004', 'AWS-006', 'AWS-007', 'AWS-008', 'AWS-009', 'AWS-012', 'AWS-018']) {
...
describe('adding the AWS track leaves the shipped tracks alone', () => {
  it('keeps every other track exactly as the labs directory declares it', async () => {
    ...
    for (const track of disk.trackIds.filter((t) => t !== 'aws')) {
  ...
  it('does not disturb the relative order of the tracks that were already here', async () => {
    ...
    // Derived from the labs directory rather than listed here, so a track
    // merged after this lab was written (cs, and whatever follows) is covered
    // instead of quietly falling outside the assertion.
    expect(order.filter((t) => t !== 'aws')).toEqual(
      disk.trackIds.filter((t) => t !== 'aws'),
    );
    expect(order).toContain('aws');
  });
