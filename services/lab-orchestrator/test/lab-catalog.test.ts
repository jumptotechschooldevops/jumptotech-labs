...
describe('catalog — discovery (test requirements 1, 2)', () => {
  it('discovers every lab in the real labs/ directory', async () => {
    ...
    // Ten Kubernetes labs from PLATFORM-003, the two PLATFORM-004 tracks, and
    // the ten Docker labs from PLATFORM-DOCKER-001.
  ...
  it('discovers every shipped track, in its declared order', async () => {
    ...
    expect(tracks.map((t) => t.labCount)).toEqual([10, 10, 1, 1]);
  ...
  it('orders labs by track then declared order', async () => {
    ...
    // …and ordering is only monotonic *within* a track. Checking per track
    // rather than globally is the point: every track restarts at 1.
